...
const items = [
  ...
  { key: "today_usage", label: "今日使用", field: "today_usage", icon: <CalendarDays className="h-4 w-4" />, color: "var(--ws-color-info, #0ea5e9)", bg: "rgba(14, 165, 233, 0.06)" },
  { key: "week_usage", label: "近7天", field: "week_usage", icon: <TrendingUp className="h-4 w-4" />, color: "var(--ws-color-danger, #ef4444)", bg: "rgba(239, 68, 68, 0.06)" },
  { key: "month_usage", label: "近30天", field: "month_usage", icon: <CalendarRange className="h-4 w-4" />, color: "var(--ws-color-accent, #8b5cf6)", bg: "rgba(139, 92, 246, 0.06)" },
] as const;
    ...
    {items.map((item) => (
      ...
      >
        ...
        <div className="min-w-0">
          <div className="text-[11px] leading-3 text-text-tertiary">{item.label}</div>
          <div className="text-base font-semibold leading-5" style={{ color: item.color }}>
