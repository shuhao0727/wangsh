...
interface AdminAILearningProps {
  embedded?: boolean;
}

interface AILearningContentPayload {
...
const CATEGORY_ICON_BY_KEY: Record<string, React.ReactNode> = {
  "llm-platforms": <Sparkles className="h-4 w-4" />,
  "dev-frameworks": <Wrench className="h-4 w-4" />,
  "vector-dbs": <BookOpen className="h-4 w-4" />,
  "image-gen": <FlaskConical className="h-4 w-4" />,
  "agent-frameworks": <Map className="h-4 w-4" />,
  evaluation: <CheckCircle2 className="h-4 w-4" />,
  "cloud-platforms": <ExternalLink className="h-4 w-4" />,
};
...
const AdminAILearning: React.FC<AdminAILearningProps> = ({ embedded = false }) => {
  ...
  const [contentPayload, setContentPayload] = useState<AILearningContentPayload | null>(null);
  ...
  const loadProgress = useCallback(async () => {
    ...
    try {
      const res = await api.get("/learning/progress/ai");
      const rawPayload = res.data?.data;
      if (rawPayload && typeof rawPayload === "object") {
        const payload = rawPayload as Record<string, unknown>;
        setProgress(() => ({
          overall_percent: (payload.overall_percent as number) ?? 0,
  ...
  useEffect(() => {
    let mounted = true;
    void fetchLearningContentPayload<AILearningContentPayload>("ai").then((payload) => {
  ...
  const saveProgress = useCallback(async () => {
    try {
      await api.post("/learning/progress/ai", {
        notes: notesText,
  ...
  }, [progress.overall_percent, progress.completedItems, progress.favoriteItems, progress.completedChapters, progress.favoriteChapters, notesText]);
  ...
  const content = (
      <div className="flex min-h-0 flex-1 flex-col gap-4">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <Sparkles className="h-6 w-6 text-[var(--ws-color-info)]" />
          <div className="min-w-0 flex-1">
            <h1 className="text-base font-bold text-text-base sm:text-lg">人工智能学习中心</h1>
            ...
            <div className="space-y-6">
              <div className="rounded-lg border border-[var(--ws-color-border-secondary)] bg-[var(--ws-color-surface-2)] p-3">
                <div className="grid gap-2 sm:grid-cols-[1fr_160px]">
                  ...
                  />
                  <Select value={experimentDifficulty} onValueChange={(value) => setExperimentDifficulty(value as typeof experimentDifficulty)}>
              ...
              <div className="rounded-lg border border-[var(--ws-color-border-secondary)] bg-[var(--ws-color-surface-2)] p-3">
                <div className="grid gap-2 sm:grid-cols-[1fr_180px]">
                  <Input
                    ...
                    className="h-9 rounded-md border border-[var(--ws-color-border)] bg-surface px-3 text-sm text-text-base placeholder:text-text-tertiary focus:border-[var(--ws-color-primary)] focus:outline-none"
                  />
                  <Select value={toolCategory} onValueChange={(value) => setToolCategory(value)}>
                    <SelectTrigger className="h-9 w-[180px]"><SelectValue /></SelectTrigger>
                    <SelectContent>
