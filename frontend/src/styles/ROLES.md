...
|------|:--:|:--:|:--:|
| 查看用户列表 | ✅ | ✅ | ❌ |
| 创建用户 | ✅ | ✅（仅 student/teacher） | ❌ |
| 编辑用户 | ✅ | ✅（仅自己+学生/教师） | ❌ |
| 删除用户 | ✅ | ✅（仅 student/teacher） | ❌ |
| 批量删除用户 | ✅ | ✅（目标缺失或任一高权限目标会整批拒绝） | ❌ |
| 导入学生/教师 | ✅ | ✅（不可导入或更新管理员） | ❌ |
...
| `api/endpoints/management/users/users.py` | 管理员保护：仅可创建、修改、导入、删除 student/teacher；批量操作锁行并原子拒绝缺失或高权限目标 |
