...
## v2.1.0 发布候选（2026-09-15，镜像标签 `2.1`）

- 应用版本统一为 `2.1.0`，六个生产镜像统一使用 `2.1` 标签；`latest` 不随本次发布更新。
- 发布范围包含本轮账号治理、点名对象级授权、请求预算、XBK 唯一性/容量裁决、
  全班级学生选课工作簿、工作簿签名预览与原子确认、前端分块恢复及网关边界治理。
- `2.0` 六镜像完整保留为回滚集；清理仅允许删除 dangling 镜像，不删除数据卷。
- 当前条目是发布候选记录：只有在工程门禁、隔离 PostgreSQL 全量测试、六镜像
  `linux/amd64` 构建、生产模拟、GitHub 推送、GitHub Actions 和 Docker Hub digest
  验证全部完成后，才可改为正式发布结果。
- 本次不部署正式服务器、不连接或迁移正式数据库。

---

## 未发布修复：AI 智能体强制正式登录（2026-09-15）

- 保留现有访客浏览模式，但 `/ai-agents` 页面新增正式登录守卫；匿名访问跳转登录页并保留安全站内返回地址，历史 `guest` 角色不能进入智能体页面。
- 后端 `/api/v1/ai-agents` 全部路由纳入认证依赖树；活跃智能体、会话历史、流式对话、使用记录写入及小组讨论配置/配置流仅允许正式登录角色，匿名返回 `401`、历史 `guest` 角色返回 `403`。
- 新增动态路由认证合同和真实 ASGI 准入回归，验证匿名请求不会触发智能体列表、历史、配置或 Provider 服务；AI 测试集 `196 passed`，前端相关专项 `41 passed`、类型检查和生产构建通过。全仓既有 lint 为 `0 errors / 468 warnings`，警告未在本轮扩大处理范围。
- 本修复无新增数据库结构或 migration。真实 Docker/Chromium 已复用本地 2.1 后端运行环境、当前前端 build 与既有 PostgreSQL/Redis 完成匿名深链、正式登录、真实 Provider 流式回复、退出失效闭环；验证过程中按既有 AUTH 文档完成开发库待执行迁移、仓库外备份和受控 authority cutover。该结果仍是本地未发布验证，不代表生产数据库已迁移或版本已发布。

---

## 未发布候选：发布前开放风险与文档门禁复核（2026-09-14）

- 只读复核确认：非默认 PostgreSQL `search_path` 的完整 Alembic 迁移链仍缺少可回放证据；
  当前迁移静态门禁还报告 XBK 最新 migration 使用动态索引名未通过审查，生产发布继续阻断。
- 教师点名在没有权威班级授权数据源前继续默认 `403`；不得仅凭 `teacher` 角色开放读取。
- 发布验收报告最低字段已同步到部署指南；动态测试事实仍以 `TEST_STATUS.md` 为唯一入口。
- 本轮未连接正常服务/数据库，未执行生产只读预检，未提交、推送、部署或修改无关代码。

---

## 未发布候选：第一批与第二阶段安全、选课一致性整改（2026-09-14）

- **公共配置**：匿名 Feature Flag 改为固定白名单和严格布尔投影，未知 key 在查询数据库前
  返回 `404`，响应禁止缓存；管理端权限不变。
- **账号治理**：停用、删除、恢复、降权与持久凭据撤销进入同一事务，并保护最后一名活跃
  超级管理员；冲突使用 `LAST_ACTIVE_SUPER_ADMIN`。创建用户会在锁后重新校验操作者；启动
  初始化只创建缺失账号，不再重置、提权或激活已存在账号。
- **点名**：学生读取按认证身份的学年与班级做对象级授权；管理员保留治理范围。教师授权
  数据源尚未建立，当前教师安全拒绝，不能视为教师点名能力已完整交付。
- **请求入口**：表单解析前增加请求头、请求体和分块等待预算；候选依赖声明更新为
  `python-multipart 0.0.32`。当前源码已构建生产候选镜像，并在一次性生产等价栈完成真实
  multipart XLSX 上传/导入回归；尚未更新正常运行环境。应用预算也不替代网关限速、并发
  控制或解析 CPU 治理。
- **XBK 迁移与并发**：手工、导入和恢复统一裁决唯一有效选课与每班容量；partial unique
  index 已在回环绑定、唯一数据库/schema 的一次性 PostgreSQL 16 中完成迁移循环、历史重复
  阻断、真实 asyncpg 约束识别及最后一个名额并发验证。一次性资源已清理，未连接或迁移正常
  数据库。
- **XBK R3 工作簿链**：已接入独立 preview/confirm HTTP 端点。preview 使用当前服务器期望值；
  confirm 从工作簿恢复不可变导出合同，并通过服务器签名 token 绑定管理员、周期和 `plan_id`，
  锁内重验完整学生、课程、选课与目标容量后整批原子应用。2026-09-14 已在一次性生产等价隔离
  环境通过完整产品 UI 导出真实工作簿，实际填写 XLSX 后再由产品 UI 文件选择器上传；preview
  返回 `200`，首次 confirm 返回 `applied`，并在 UI 选课总表看到结果。同一签名 confirm 请求重试返回
  `already_applied`；重新上传旧工作簿因导出基线过期在 preview 阶段返回预期 `422`。真实
  PostgreSQL 回查未发现重复有效选课、课程超额或孤儿选课。
- **XBK 学年选择**：修复服务器仅返回历史学年时，当前筛选学年不在选项中导致下拉显示空白的
  问题；一次性生产等价隔离前端已重建，并经真实 Chromium 复验当前学年显示、历史学年选择及
  工作簿应用结果均正常。
- **前端切换观察项**：新增 `vite:preloadError` 恢复逻辑已在一次真实 Chromium 隔离观察中触发
  一次自动刷新，并由 `sessionStorage` 防止无限刷新，最终进入错误边界；独立二次复验和正式验收报告
  尚未完成，不能把本轮观察扩称为完整切换验收。
- **迁移风险**：非默认 PostgreSQL `search_path` 下，Alembic 版本表兼容逻辑的固定
  `public` 检查与未限定 schema DDL 可能触发 `DuplicateTableError`；XBK 最新 migration 的索引名
  已改为静态字面量并通过静态迁移门禁，但自定义 schema/多租户迁移仍需真实专项验收。
- **发布状态**：当前源码的生产候选镜像已经重建，并在独立 PostgreSQL 16、Redis 7、双
  Uvicorn worker、正式 bootstrap/Alembic/AUTH cutover、真实 gateway、HTTP、XLSX、完整产品 UI
  与 Chromium 组成的一次性生产等价环境完成验证。该结论仅表示“当前源码生产等价一次性环境
  通过”；候选仍未提交、推送或部署，未处理真实业务数据，也未迁移正常数据库，不代表生产环境
  已经修复。更新前仍需受控预检、备份、迁移和正式生产验收，不能宣称全部漏洞已经关闭。

---

## 未发布修复：认证异地登录误报与重复提示（2026-09-14）

- 后端在 Redis nonce 与 access token 不一致时重新读取 PostgreSQL 持久会话权威；只有数据库 nonce 确已轮换才返回“账号已在其他地方登录”，Redis 投影陈旧不再误判为新登录。
- 前端认证失效详情改为 60 秒内、同标签页的一次性事件：不再写入长期 `localStorage`，展示后立即消费，普通登录入口、刷新与 StrictMode 重挂载不会复活历史提示。
- 验证：后端认证测试 `407 passed / 5 skipped`；前端认证测试 `132 passed`，TypeScript 类型检查通过；本地真实双浏览器登录确认第二次登录后旧会话 `/auth/me` 与 `/auth/refresh` 均为 `401`、只提示一次，再次刷新不重放且旧 token 已清除。
- 无数据库结构变更或 migration；代码纳入本次 `main` 发布提交，Docker 镜像与生产部署状态以对应发布 workflow 和 release-set 验证结果为准。

---

## 未发布修复：AI 智能体课堂互动入口（2026-09-14）

- AI 智能体页面重新向管理员和超级管理员显示“课堂互动”浮动按钮，点击后进入现有
  `/admin/classroom-interaction` 管理页面。
- 学生端课堂轮询、课堂计划查询、SSE 和答题权限保持不变；管理员入口不会调用学生接口，
  避免出现 `403` 和 SSE 重连。

---

## 未发布修复：课堂计划启动错误定位与失效恢复（2026-09-14）

- 课堂计划管理接口统一区分错误语义：计划不存在返回 `404`，对象级权限失败返回 `403`，状态、空计划和班级范围校验失败返回 `400`。
- 缺少班级、班级不一致或计划引用活动丢失时，响应会列出具体活动 ID、标题和班级归属。已结束历史活动不开放任意编辑，安全恢复路径为复制活动、在草稿副本中补充班级并重新加入计划。
- 管理端遇到计划 `404` 时会关闭陈旧编辑区或控制台并刷新列表；`400` 校验错误仍保留控制台并展示具体恢复建议。活动详情侧栏补充无障碍描述，API 控制台错误首行可直接查看净化后的方法、路径、状态码和服务端详情。
- 本轮没有数据库结构变更或 migration，不修改历史数据；代码纳入本次 `main` 发布提交，Docker 镜像发布以对应 workflow 结果为准，生产部署仍需后续单独授权与验收。

---

## v2.0.0 发布候选（2026-09-11，镜像标签 2.0）

- **认证**：持久会话权威闭环（DB 为撤销权威、受控 enrollment、S7 转发头可信网段治理）；真实代理链 8 场景验收通过（见 [归档](archive/2026-09-11-proxy-chain-acceptance.md)）。
...
## 未发布修复：Gateway 边界治理（2026-09-15）

- 生产候选 Caddy 由 2.8.4 升级到 2.10.2，并增加`64KB` 请求头上限、请求读取/写响应/空闲连接超时，以及反代连接和上游响应超时。
- 未加入未经验证的通用限速模块；公网 IP/并发限速仍需由 WAF/LB/API Gateway 提供证据。
- 已增加 `gateway/tests/test_caddy_config.sh` 配置合同检查；本次仅进行独立 Docker 隔离验证，不接触正常端口、不部署生产。
- 后续隔离复测确认：超限请求客户端最终收到 `413`，但部分请求可能先将约 16 MB 前缀送达 upstream；因此“超限零转发”和通用 IP/并发限速仍为发布阻断项，需由受信任 WAF/LB/API Gateway 或经验证的缓冲层补足。

## 2026-09-15 — 2.1 本地生产模拟 AUTH enrollment 修复

- 本地 `scripts/deploy.sh simulate` 在一次性隔离数据库完成迁移和健康检查后，现会调用产品
  `bootstrap_durable_auth_authority(..., legacy_writers_stopped=True)`，并使用独立 session 查询
  `auth_authority.ready`，确认开启后才运行需要登录的完整 smoke。
- 该自动步骤只适用于 simulate 创建的空白、一次性、无 legacy writer 数据库；正式数据库仍必须
  通过 `backend/app/api/endpoints/auth/cutover.py` 执行停流、排空、配对快照和显式 apply，不能复用
  模拟夹具。
- 修复前真实 `:2.1` 模拟中健康检查通过，但 13 个 smoke 因登录 fail-closed 返回 503 而失败；
  该失败证据保留在本地临时测试产物中。
- 修复后于 2026-09-15 重新完整运行生产模拟，结果为
  `14 steps / 12 pass / 2 warn / 0 fail / 0 skip`。两个 WARN 分别来自模型发现只读端点按权限
  返回 `401`，以及空白模拟库没有稳定学习内容/ML book 可做原数据恢复；核心业务、工作进程和
  产品 UI 均通过。`SIM_CLEANUP=true` 已清除本轮隔离容器、网络和数据卷。
- 修复 `verify-release-set` 把完整应用版本 `2.1.0` 与镜像标签 `2.1` 强制做字符串相等比较的
  发布阻断；现在以 `frontend/package.json` 为版本权威源，分别校验完整应用版本与 major.minor
  镜像标签，并增加真实 `2.1.0 / 2.1` 接受及多类漂移拒绝合同测试。
