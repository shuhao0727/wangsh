...
- 按明确路径清单复核 Git 归属、历史、引用、大小和 SHA，再将可重建的 Finder 元数据及
  Python 字节码备份到仓库外后逐文件删除；不递归清空缓存目录。
- 源码、owner 文档、迁移、备份恢复资料、当前截图及接力摘要没有充分删除依据，继续保留。
  活跃 pytest/Ruff 缓存不清理，不以“文件旧”推断无用。
- 已校正当前缺失备份/演练入口的使用说明，保留运维要求及历史出处，不直接恢复未经验证的
  历史脚本。项目目录结构和业务文件未作批量移动。
- 本批 exact manifest、外部可恢复备份、跳过项和执行结果见
  [TEST_STATUS](../testing/TEST_STATUS.md) 的四线审查记录。只统计实际删除，不把候选当结果。
