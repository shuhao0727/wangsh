...
| 问题 | 原状态 | 当前状态 |
|------|--------|----------|
| 游客访问文章页 401 | P1 未解决 | **已解决 v1.5.3** — 公开端点已实现 |
| Feature Flag 页面级拦截 | P1 未解决 | **已实现** — sys_feature_flags 表 + JSONB |
| 测评系统缺 mock 测试 | P1 未解决 | **已实现** — test_assessment_profile/session.py |
| pub/sub 提取到独立模块 | P1 未解决 | **已完成** — app/core/pubsub.py |
| main.py 职责过重 | P1 未解决 | **已完成** — lifespan 拆分到 app/core/startup.py |
| SSE 单进程限制 | P0 未解决 | **已解决 v1.5.4** — 改用 Redis pub/sub，支持多 worker |
| 大文件拆分 (data.py/flow.py) | P2 未解决 | **未解决** — data.py 645行, flow.py 1277行 |
| CSS 文件清理 | P2 未解决 | **部分完成** — 15个减至8个 |
| 版本号不一致 | P0 未解决 | **已解决 v1.5.3** — 全部统一为 1.5.3 |
