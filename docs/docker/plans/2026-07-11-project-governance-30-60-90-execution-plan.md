...
[发布与运维记录](../RELEASE_NOTES.md)，实时测试事实见
...
- [x] `origin/main` 已完成发布基线验证，通用 GitHub Actions 通过。
- [x] 六镜像 `release-set` 已通过本机构建、隔离模拟和远端 digest 复核。
- [ ] 当前文件整理批次提交后，从最终提交重新运行本地门禁。
- [ ] 正式生产部署前完成数据库升级、恢复和回滚演练；本轮按用户要求不执行数据库备份。
- [ ] 当前收口计划完成后归档，未完成事项转入对应 owner。

长期治理可以先执行不依赖生产环境的代码、warning 和文档工作，但不得把未完成的数据库
演练写成发布完成条件。
