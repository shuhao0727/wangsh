...
阶段报告和旧计划在长期结论沉淀后转为 archive 或 redirect。代码按八个原子提交收口，
每批先验证再提交，最后执行完整本地门禁、远端 PR 和发布验收。
...
Markdown 整理已经完成，剩余动作仅是按 Commit 8 边界显式暂存、从最终 HEAD 重跑门禁，
以及在真实 GitHub runner 上验证新 workflow；本轮不新增第二份整理报告。
...
1. 审查并显式暂存 Commit 6 范围。
2. 完成管理员凭据轮换后提交 Commit 6。
3. 继续 Commit 7、Commit 8 和最终 HEAD 生产模拟。
...
| 工作区 | Commit 1-5 已提交；Commit 6-8 仍在工作区 |
| 已暂存 | 空 |
| 已提交 | Commit 1-5 |
...
| Markdown | 120 份、263 个相对链接、0 断链 |
| 发布状态 | v1.6.0 尚无远端 Git tag 和 Docker Hub 六镜像 release-set |

Commit 1-5 已提交。当前未提交增量完成后端全量、前端全量、TypeScript、lint、生产
构建、前端脚本、workflow contracts、Compose、Python governance、Markdown 链接和
格式检查；远端 runner、registry 和最终生产模拟仍需在 Commit 6-8 后执行。
...
```bash
git commit -m "ci: enforce PythonLab and production release contracts"
```
...
```bash
git commit -m "chore: remove verified dead code and generated artifacts"
```
...
```bash
git commit -m "docs: synchronize governance and release guidance"
```

## 六、阶段 4：完整本地发布门禁

八个提交完成后，从分支 HEAD 重新运行，不复用 dirty worktree 结果：
...
- [ ] 推送 `codex/project-governance-release-1.6.0`。
- [ ] 创建 PR，不直接合并。
- [ ] 等待通用 CI 和 PythonLab PR runtime 全部通过。
...
1. 审查并提交 Commit 6。
2. 审查 25 个删除入口并提交 Commit 7。
3. 整理全部 Markdown，形成 Commit 8。
4. 从分支 HEAD 重跑完整本地门禁和生产模拟。
5. 推送 PR，执行真实 GitHub Actions。
6. 发布六镜像和 release-set。
7. 完成数据库演练、文档归档和最终报告。
