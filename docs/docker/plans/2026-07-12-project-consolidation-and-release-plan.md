...
Markdown 整理已经完成并进入 `79a0c95`。剩余动作是从最终 HEAD 重跑门禁，
推送分支并在真实 GitHub runner 上验证新 workflow；本轮不新增第二份整理报告。
...
1. 从 `79a0c95` 及后续文档校正提交的最终 HEAD 重跑完整门禁。
2. 完成管理员凭据轮换，不把本地或远端有效凭据写入仓库。
3. 推送分支、创建 PR，并等待真实 GitHub runner 验证。
...
| 工作区 | 干净；原 Commit 6-8 范围已由 `79a0c95` 合并提交 |
| 已暂存 | 空 |
| 已提交 | Commit 1-6；Commit 6 为综合收口提交 `79a0c95` |
...
Commit 1-6 已提交，工作区干净。此前 dirty worktree 结果只作为阶段证据；后端、前端、
脚本、workflow contracts、Compose、Python governance、Markdown 和格式检查仍须从
最终分支 HEAD 重新执行。远端 runner、registry 和生产迁移演练仍未完成。
...
提交收口后，从分支 HEAD 重新运行，不复用 dirty worktree 结果：
...
1. 从最终分支 HEAD 重跑完整本地门禁。
2. 提交验证后产生的文档准确性修正。
3. 推送分支并创建 PR，执行真实 GitHub Actions。
