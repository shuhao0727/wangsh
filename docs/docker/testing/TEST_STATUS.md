...
下表记录截至 2026-07-14 的 `main` 主线验证。治理提交与 clean-runner 修复已推送，
本地门禁、通用 GitHub Actions 和 Markdown 质量门禁均已通过；本页不把这些结果
外推为 PythonLab 专项 runtime、registry 或正式生产结果。
...
| GitHub Actions | 通用 CI 与 Markdown 成功 | `main` push 的 repo、backend、frontend、docs jobs | 远端已通过 |
...
`30 passed`、`git diff --check` 通过。

修复提交 `17da07b` 的远端结果：

- `ci-quality` run `29310765657`：`repo-config`、`backend-pytest`、
  `frontend-quality` 全部成功。
- `markdown-quality` run `29310765680`：成功。
- 远端仅保留非阻断 annotation：历史 ESLint warning，以及 GitHub Actions 对
  Node 20 action runtime 的弃用提醒。
