...
| 类别 | 当前结果 | 覆盖内容 |
|---|---|---|
| 后端全量 | `696 passed, 1 skipped, 9 warnings` | API、服务、迁移、脚本与核心业务回归；唯一 skip 是专用测试库保护 |
| 前端全量 | `75 files / 362 passed` | 组件、页面、状态与工具函数回归，包含 `src/lib` 运行时边界和 Assessment 静态新建路由 |
...
| 前端脚本 | `27 passed` | UI smoke 页面/总控状态、构建上下文、token、Pyodide、Git 白名单和最终产物合同 |
...
| Python governance | `0 errors / 5 warnings` | 文件规模与复杂度 ratchet |
| Markdown 链接 | `104 files / 253 links / 0 missing` | owner、索引、归档和相对链接 |
...
- `pull-up/deploy` 在拉取后逐个核对本地镜像 `RepoDigests`；`up-no-build` 也拒绝
  ...
  部署健康入口同时验证站点首页和 API。
- 本批尚未 commit/push；本地 `main` 在 2026-07-22 刷新远端引用后仍领先
  `origin/main` 3 个已有提交。远端 GitHub Actions 和已发布镜像不包含本轮工作区修改。
