...
下表记录截至 2026-07-13 的当前分支和剩余工作区本地验证。Commit 1-5 已提交，
Commit 6-8 完成后仍须从最终分支 HEAD 重跑完整门禁；不得把定向测试外推为远端发布结果。
...
| Workflow contracts | `28 passed` | GitHub Actions 路径、依赖、日志脱敏和发布合同 | 已通过 |
...
### Commit 6：CI、部署合同和日志脱敏

2026-07-13 对当前工作区的 CI/脱敏增量完成独立复核：
...
- Commit 6-8 完成后，从分支 HEAD 重跑后端、前端、脚本、Compose 和治理全量门禁。
