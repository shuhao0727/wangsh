...
构建的镜像列表：
- `shuhao07/wangsh-backend:1.5.10` - 后端 FastAPI 服务
- `shuhao07/wangsh-frontend:1.5.10` - 前端静态文件
- `shuhao07/wangsh-gateway:1.5.10` - Caddy 网关
- `shuhao07/wangsh-typst-worker:1.5.10` - Typst PDF 编译 worker
- `shuhao07/wangsh-pythonlab-worker:1.5.10` - PythonLab 调试 worker
...
### 版本配置
```bash
APP_VERSION=1.5.10          # 应用版本号
IMAGE_TAG=1.5.10            # Docker 镜像标签
REACT_APP_VERSION=1.5.10    # 前端版本号
```
