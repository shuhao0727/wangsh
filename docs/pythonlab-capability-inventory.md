...
- 当前普通运行可走 Pyodide 或远端会话；当前断点调试已收敛为 DAP 唯一路径。
...
| 语法检查 | 编辑器输入后自动检查 | `POST /api/v2/pythonlab/syntax/check` | `frontend/src/pages/Admin/ITTechnology/pythonLab/components/rightPanel/RightPanelView.tsx`<br>`frontend/src/pages/Admin/ITTechnology/pythonLab/services/pythonlabCodeApi.ts`<br>`backend/app/api/pythonlab/syntax.py` | 前端 800ms debounce 调后台语法检查，并将错误标记回 Monaco。 |
| 断点设置与显示 | 点击编辑器左侧行号 / gutter | 无直接 API；调试启动后走 DAP `setBreakpoints` over WS | `frontend/src/pages/Admin/ITTechnology/pythonLab/components/MonacoPythonEditor.tsx`<br>`frontend/src/pages/Admin/ITTechnology/pythonLab/PythonLabStudio.tsx`<br>`frontend/src/pages/Admin/ITTechnology/pythonLab/hooks/useUnifiedRunner.ts`<br>`frontend/src/pages/Admin/ITTechnology/pythonLab/hooks/useDapRunner.ts`<br>`frontend/src/pages/Admin/ITTechnology/pythonLab/hooks/usePyodideRunner.ts` | 断点状态先保存在前端；Pyodide 只保留断点展示与“点击调试切 DAP”入口，不再执行本地调试语义。 |
...
| 调试启动 | 控制栏 `Debug` 按钮 | `POST /api/v2/pythonlab/sessions`、`GET /api/v2/pythonlab/sessions/{id}`、`WS /api/v2/pythonlab/sessions/{id}/ws` | `frontend/src/pages/Admin/ITTechnology/pythonLab/PythonLabStudio.tsx`<br>`frontend/src/pages/Admin/ITTechnology/pythonLab/launchPlan.ts`<br>`frontend/src/pages/Admin/ITTechnology/pythonLab/hooks/useDapRunner.ts`<br>`frontend/src/pages/Admin/ITTechnology/pythonLab/hooks/usePyodideRunner.ts`<br>`frontend/src/pages/Admin/ITTechnology/pythonLab/services/pythonlabSessionApi.ts`<br>`backend/app/api/pythonlab/sessions.py`<br>`backend/app/api/pythonlab/ws.py` | 当前要求至少有 1 个启用断点；若 `sourceMismatch`，会先尝试重建流程图映射；断点调试已不再回落到 Pyodide。 |
| 调试继续 / 暂停 / 单步 / 重置 | 控制栏 `Pause / Continue / Step Over / Step Into / Step Out / Reset` | 通过 `WS /api/v2/pythonlab/sessions/{id}/ws` 转发 DAP 请求 | `frontend/src/pages/Admin/ITTechnology/pythonLab/PythonLabStudio.tsx`<br>`frontend/src/pages/Admin/ITTechnology/pythonLab/components/rightPanel/RightPanelView.tsx`<br>`frontend/src/pages/Admin/ITTechnology/pythonLab/hooks/useUnifiedRunner.ts`<br>`frontend/src/pages/Admin/ITTechnology/pythonLab/hooks/useDapRunner.ts` | 控制矩阵由 `runner.status` 驱动；Pyodide 不再暴露这些伪调试控制。 |
| 变量面板 | 右侧面板 `调试器` tab 的“变量”区 | 无独立 HTTP API；DAP 走 `scopes/variables` over WS | `frontend/src/pages/Admin/ITTechnology/pythonLab/components/rightPanel/DebugTab.tsx`<br>`frontend/src/pages/Admin/ITTechnology/pythonLab/hooks/useDapRunner.ts` | 暂停时刷新变量；支持 changed vars 高亮；非 DAP 运行模式只显示不可用提示。 |
| Watch 表达式与求值 | 右侧面板 `调试器` tab 的“表达式”区 | 无独立 HTTP API；DAP 走 `evaluate` over WS | `frontend/src/pages/Admin/ITTechnology/pythonLab/components/rightPanel/DebugTab.tsx`<br>`frontend/src/pages/Admin/ITTechnology/pythonLab/hooks/useDapRunner.ts` | 仅在暂停时计算；非 DAP 运行模式只显示不可用提示。 |
...
| 风险 | 严重度 | 当前代码证据 | 冻结结论 |
|---|---|---|---|
| Pyodide worker 的历史兼容补丁复杂度 | P0 | `usePyodideRunner.ts` 当前显式以 `{ type: "module" }` 创建 `pyodideRunner.worker.ts`；worker 已收缩为 plain-run + `input()` bridge，但仍承载 Pyodide 加载细节。 | 风险已从“调试协议冲突”收敛到“本地运行 worker 仍较重”；后续若继续收敛架构，可再评估是否进一步拆薄。 |
...
| Pyodide 版本锁定 | 检查 `frontend/package.json`、`frontend/public/pyodide/pyodide-lock.json`、`pyodideRunner.worker.ts`。 | `PASS` | 当前项目已把 Pyodide 版本锁到 `0.25.1`，对应 Python `3.11.3` 与 `emscripten_3_1_46`；版本来源是清晰的。 |
...
- 历史实测结果：
  - 页面曾直接显示 `Pyodide 未就绪`
  - 浏览器控制台曾报错：`Cannot use import statement outside a module`
  - 截图已保存：`output/playwright/pythonlab-pyodide-not-ready.png`
- 历史代码证据：
  - `usePyodideRunner.ts` 曾通过旧版 Pyodide worker 名称创建 worker，且未显式传 `{ type: "module" }`
  - dev server 暴露过旧 worker 的 `?worker_file&type=module` 资源
  - 当时 worker 内还保留 `importScripts shim`
- 结论：
  - 该问题已经是历史问题，不再代表当前主干状态。
  - 当前代码已显式使用 module worker，且 Pyodide worker 只承担 plain-run。
  - 2026-04-08 当天的 `pythonlab-debug-smoke.mjs` 已通过普通运行、`input()` 远端运行和完整 DAP 调试链路验证。
