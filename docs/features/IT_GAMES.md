...
上传目录在首次上传时自动创建。上传先写入同目录临时文件，完成格式校验后使用 `os.replace` 原子切换为最终文件。删除时先把物理文件移入隔离名，数据库提交成功后再清理；提交失败会恢复文件。
...
| `GET` | `/it/games/categories` | 获取所有分类列表（去重排序） |
...
- 每次下载自动记录：用户 ID、客户端 IP、浏览器 User-Agent
