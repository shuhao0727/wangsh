...
管理和学生课堂事件流在订阅前、输出下一条事件前及空闲等待中复查入场 JWT 与服务端会话。退出导致 nonce 失效、JWT 到期、存储异常/超时或认证上下文缺失时结束流，取消待取队列任务并退订。班级频道及原事件格式不变。

此机制只保证校验点拒绝失效会话，不保证退出与网络发送原子同步；在途帧、代理缓冲、慢连接及后续角色/班级变更不在即时撤销保证内。策略与时限见 [认证文档](AUTH.md#管理与课堂-sse-会话持续校验)，实际回归与未覆盖项见 [TEST_STATUS](../docker/testing/TEST_STATUS.md)。
...
课堂计划没有独立班级字段，因此创建或更新时，所有活动必须设置非空 `class_name`，并且必须属于同一班级。启动草稿计划时会再次校验这一约束，避免历史混合班级计划进入 active 后对学生不可见。缺少班级时，错误会列出对应活动 ID 和标题；存在多个班级时，错误会按班级列出对应活动，便于教师直接定位问题。

计划不存在统一返回 `404`，对象级权限失败返回 `403`，状态不允许、空计划、活动缺少班级或班级不一致等业务校验失败返回 `400`。已结束的历史活动仍不允许直接编辑；如果历史计划引用了未设置班级的已结束活动，应复制该活动，在草稿副本中补充班级，再把副本加入计划。将计划从 `ended` 重置为 `draft` 只重置状态，不会绕过启动时的班级校验。
...
管理端计划页面会区分失效对象与业务校验：计划接口返回 `404` 时关闭对应的陈旧编辑区或控制台、刷新计划列表并去重提示；`400` 班级范围错误仍保留控制台，并直接展示后端返回的活动定位和恢复建议。活动详情侧栏包含无障碍描述，API 错误日志首行只记录请求方法、去除查询参数后的路径、状态码和服务端详情，不记录认证头、Cookie、token 或请求正文。
