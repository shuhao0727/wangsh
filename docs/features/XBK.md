...
## 第一、二阶段未发布候选：统一裁决、统一工作簿与唯一约束（2026-09-14）

本节记录已进入源码并完成隔离生产等价验证的候选行为，不表示正常数据库已迁移，也不表示
生产环境已经发布。

### 统一事务裁决与父对象守卫（R1/R2）

- 手工新增、改课、选课导入和软删除恢复统一进入事务裁决服务；同一学年、学期、学号
  最多保留一个有效选课位置。导入替换旧位置而不是新增第二条有效记录，同一批次为同一
  学生提交多个最终课程时整批拒绝。
- 课程容量按 `(year, term, course, roster.grade, roster.class_name)` 独立裁决，限额是每班
  容量，不是跨班总人数。学生转班或转年级时重验目标班容量。
- 学生或课程存在任何历史选课关联时，拒绝修改会破坏关联的自然键。选课更新在取得锁后
  还会核对不可变源键；若等待期间记录已被并发修改，返回 `409` 并要求刷新。
- 候选唯一约束触发的数据库 `IntegrityError` 在手工新增/修改入口回滚并映射为 `409`，
  不应作为 `500` 暴露。上述保证只覆盖已接入统一服务的 writer；维护脚本、直接 SQL 和
  混合旧版本实例仍需在发布前逐项清点。

### 全班统一选课工作簿（R3）

- `course-selection` 导出按实际名册动态生成全部班级工作表，不写死班级数量；跨年级同名
  班级不会合并。班级表仅解锁学生课程代码输入格，剩余名额按本班计算，且不包含“顺序”列。
- 隐藏的版本化元数据记录学年、学期、基线摘要、工作表映射、完整名册、基线课程及课程
  目录。解析器遍历全部班级页，校验身份、基线、课程代码和跨 Sheet 重复，并返回包含工作表、
  行、学生、课程、原因及建议的诊断。空白表示不变，`未选` 表示显式取消。
- R3 已接入独立的管理员 multipart HTTP 预览/确认端点。预览使用当前服务器基线生成稳定
  `plan_id` 和服务器签名 `preview_token`；确认从同一工作簿恢复不可变导出合同，重新计算计划
  并匹配签名 token，随后在数据库锁内核验完整有效学生集合、课程代码/名称/限额及当前选课。
  完整原基线执行整批修改，完整目标状态幂等返回 `already_applied`；新增/缺失学生或课程、计划
  篡改、第三状态、部分应用或一名学生存在多条有效选课均整批拒绝。失败或任务取消时回滚。
- 现有 `POST /xbk/import/preview` 与 `POST /xbk/import` 不识别 R3 工作簿。工作簿显示的剩余名额
  只是导出快照，不预留名额；最终容量与唯一有效位置仍由确认事务裁决。2026-09-14 已在当前
  源码构建的一次性生产等价隔离环境中，通过完整产品 UI 导出真实工作簿，实际填写 XLSX 后再由
  产品 UI 选择文件上传；preview 返回 `200`，首次 confirm 返回 `applied`，并在 UI 选课总表看到结果。同一签名 confirm 请求重试返回
  `already_applied`；重新上传已应用的旧工作簿时，因导出基线过期在 preview 阶段返回 `422` 并
  安全拒绝。真实 PostgreSQL 终态回查未发现重复有效选课、课程超额或孤儿选课。
- 学年下拉在服务器仅返回历史学年、当前筛选学年不在选项中时曾显示空白；修复后已重建一次性
  生产等价隔离前端，并用真实 Chromium 复验当前学年可见、历史学年仍可选择，完整工作簿结果仍
  可查询。以上仅代表当前源码生产等价一次性环境通过，不代表代码已提交、系统已部署或生产环境
  已经修复。
- 已分别用 Microsoft Excel 和 WPS 实际打开、保存、关闭后重新解析测试工作簿；两者均保留
  工作表保护、课程代码输入格可编辑和 `veryHidden` 元数据页。Excel 会把标准
  `dataValidations` 改写为 `x14:dataValidations` 扩展，WPS 则保留标准 `dataValidations`；
  openpyxl 会警告不支持 Excel 的 x14 扩展，因此不得再用 openpyxl 保存 Excel 往返文件，
  否则可能移除验证规则。上述往返只验证文件兼容性，不等于 HTTP 回导或数据库写入已开放。

### PostgreSQL 候选迁移（R4）

- 候选 revision `20260914_0001_xbk_active_selection_unique` 拟为
  `(year, term, student_no)` 建立 `is_deleted IS FALSE` 的 PostgreSQL partial unique index
  `uq_xbk_selections_active_period_student`。
- upgrade 先以 `SHARE ROW EXCLUSIVE` 锁定 `xbk_selections`，再检查活跃重复；发现冲突即
  阻断并报告最多 20 个样本，不自动删除、合并或退选历史数据。downgrade 也取得同级写锁。
- 该 revision 已在回环地址的一次性 PostgreSQL 16、唯一测试数据库和隔离 schema 中完成
  `upgrade -> downgrade -> upgrade`。合成历史有效重复会阻断 upgrade，失败事务回滚且不留下
  半成品索引；显式处理合成重复后可以重新升级。真实 asyncpg 冲突能返回目标约束名，应用
  只把 `uq_xbk_selections_active_period_student` 识别为 R4 冲突，旧四字段唯一约束不被误判。
- 实际 partial unique index 已验证：同一学生、学年、学期不能保留两条有效选课，软删除记录
  不参与唯一限制；最后一个班级名额的并发竞争在锁后复核，只有一人成功，另一人返回 `409`。
- **以上均为一次性专库和合成数据验证，正常数据库未连接、未迁移，服务未部署。** 一次性
  容器、卷和环境文件已清理。正式发布仍须在批准的停写窗口内审计并处理真实重复数据、完成
  备份恢复准备，并按部署 owner 文档执行迁移。
