...
async def list_published_mindmaps(db: AsyncSession) -> list:
    """公共导图广场。"""
    stmt = (
...
async def list_my_mindmaps(db: AsyncSession, user_id: int) -> list:
    """我的个人导图。"""
    stmt = (
        select(LearningContentItem)
        .where(
...
async def create_mindmap(db: AsyncSession, user_id: int, body: MindmapCreate) -> dict:
    """创建个人思维导图。"""
    now = datetime.utcnow()
...
async def update_mindmap(
    db: AsyncSession, user_id: int, mindmap_id: int, body: MindmapUpdate
) -> dict:
    """更新个人思维导图（仅所有者）。"""
...
async def delete_mindmap(db: AsyncSession, user_id: int, mindmap_id: int, is_admin: bool) -> dict:
    """删除个人思维导图（所有者或管理员）。"""
    stmt = select(LearningContentItem).where(LearningContentItem.id == mindmap_id)
    result = await db.execute(stmt)
    item = result.scalar_one_or_none()
...
async def toggle_mindmap_publish(db: AsyncSession, mindmap_id: int) -> dict:
    """管理员发布/取消发布导图。"""
    stmt = select(LearningContentItem).where(LearningContentItem.id == mindmap_id)
    result = await db.execute(stmt)
    item = result.scalar_one_or_none()
