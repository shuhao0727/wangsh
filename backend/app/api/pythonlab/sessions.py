...
def _session_ws_path(request: Request, session_id: str) -> str:
    path = str(request.url.path or "").strip()
    prefix = path.rsplit("/sessions", 1)[0] if path.endswith("/sessions") else ""
    if not prefix:
        prefix = PYTHONLAB_V2_PREFIX
    return f"{prefix}/sessions/{session_id}/ws"
...
class DebugLimits(BaseModel):
    ...
    memory_mb: int = Field(
        default_factory=lambda: max(32, int(getattr(settings, "PYTHONLAB_DEFAULT_MEMORY_MB", 512) or 512)),
        ge=32,
        le=4096,
    )
...
async def create_session(request: Request, payload: DebugSessionCreateRequest, current_user: Dict[str, Any] = Depends(require_user)):
    ...
    return DebugSessionCreateResponse(
        session_id=session_id,
        status=SESSION_STATUS_PENDING,
        ws_url=_session_ws_path(request, session_id),
    )
