...
def test_delete_compatible_siblings_only_deletes_the_same_batch():
    ...
    kept = HotQuestionAnalysis(
        **fields,
        result={"analysis_version": "hot_v2", "batch": "first"},
        created_at=first_created_at,
    )
    first_sibling = TaskAnalysis(
        **fields,
        result={"analysis_version": "hot_v2", "batch": "first"},
        created_at=first_created_at,
    )
    second_sibling = TaskAnalysis(
        **fields,
...
def test_delete_compatible_siblings_keeps_ambiguous_exact_matches():
    ...
    kept = HotQuestionAnalysis(**fields, result=result, created_at=created_at)
    candidates = [
        TaskAnalysis(**fields, result=result, created_at=created_at),
        TaskAnalysis(**fields, result=result, created_at=created_at),
    ]
...
def test_legacy_record_is_not_used_as_a_sibling_cleanup_source():
    fields = _business_fields()
    created_at = datetime(2026, 7, 13, 2, 1, tzinfo=timezone.utc)
