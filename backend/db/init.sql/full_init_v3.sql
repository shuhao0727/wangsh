...
DO $$
BEGIN
    RAISE NOTICE '🚀 开始数据库完全初始化（v3.0）...';
    RAISE NOTICE '    时间: %', NOW();
    RAISE NOTICE '    目标：创建增强数据保护机制的数据库';
    RAISE NOTICE '    特性：快照保护、软删除、完整历史记录';
END $$;
...
DO $$
BEGIN
    RAISE NOTICE '✅ 12个核心表创建完成（v3.0增强版）';
END $$;
...
INSERT INTO alembic_version (version_num) VALUES ('full_init_v3_20260206');
